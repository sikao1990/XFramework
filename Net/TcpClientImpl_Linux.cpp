...
TcpClientImpl::TcpClientImpl():m_client(NULL),m_flag(false),m_bStart(false)
{
    m_pNet = new AsyncNetClientImpl(2);
...
void TcpClientImpl::Run()
{
	...
	CPostReq req={};	
	while(m_bStart){
		if(m_pNet->GetQueueComplateStatus(sock,req,nTrans)){
			switch(req.oType){
			case OP_READ:
				break;
			case OP_WRITE:
...
bool TcpClientImpl::PostRecvReq(const hSockFd& fd,char* pStart,int len,int nIndex,void* pExtend,void* pPostNode )
{
    ...
    pReq->cSock = fd;
    m_pNet->postQuest(fd, jobtype(READTASK),*pReq);
    return true;
...
bool TcpClientImpl::PostSendReq(const hSockFd& fd,char* pStart,int len,int nIndex,void* pPostNode )
{
    CPostReq* pReq = (CPostReq*)pPostNode;
    pReq->cSock = fd;
    m_pNet->postQuest(fd, jobtype(WRITETASK),*pReq);
    return true;	
}
