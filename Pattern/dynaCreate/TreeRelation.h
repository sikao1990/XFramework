...
#define ALLOCMEM(Size)		(new char[Size])
...
#define DEFINENODE(base,type,kind,parent,AllocMem) 		\
	void* type::Create##type##Instance(){	\
		return new(AllocMem(sizeof(type)))type;		\
	}										\
	...
	extern "C" {							\
		bool Init##type(){						\
			if(strcmp(#base,#type))AddToParent(base,type);\
		}\
	}\
